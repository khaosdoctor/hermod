...
export async function createConnection ({ hostname, port, username, password, protocol, maximumConnectionAttempts, connectionRetryInterval }: IRabbitMQParams, attempsMade: number = 1): Promise<Connection> {
  try {
    ...
    if (attempsMade >= maximumConnectionAttempts + 1) throw new ConnectionError(err.message)
